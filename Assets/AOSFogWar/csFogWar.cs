    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        private void OnDrawGizmos()
        {
            if (Application.isPlaying == false)
            {
                ...
                var posX = Vector3.right * (levelDimensionX * unitScale) / 2f;
                var posZ = Vector3.forward * (levelDimensionY * unitScale) / 2f;
                Vector3 pos = levelMidPoint.position;

                Gizmos.DrawLine(pos - posX - posZ, pos - posX + posZ);
                Gizmos.DrawLine(pos - posX - posZ, pos + posX - posZ);
                Gizmos.DrawLine(pos + posX + posZ, pos - posX + posZ);
                Gizmos.DrawLine(pos + posX + posZ, pos + posX - posZ);
                return;
